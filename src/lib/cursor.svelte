<script>
	...
	onMount(async () => {
		const module = await import('https://esm.sh/vecteur');
		vec2 = module.vec2;
		...
		class Cursor {
			...
			// addListeners() {
			// 	gsap.utils.toArray('.cursor-hover').forEach((hoverEl) => {
			// 		// set hover states
			// 		{
			// 			const hoverBoundsEl = hoverEl.querySelector('.cursor-hover__bounds');
